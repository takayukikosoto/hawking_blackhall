# 物理計算の実装詳細

## 概要

Hawking Radiation Simulator Proで実装されている物理計算の詳細ドキュメントです。

## 数値計算手法の分類

本シミュレーターは、天体物理学で標準的に使用される数値計算手法を組み合わせて実装されています。

### 使用している手法

| 数値計算法 | 用途 | 実装状況 |
|-----------|------|---------|
| **粒子法 (Particle Method)** | パーティクルシステムによるホーキング放射の可視化 | ✅ 実装済み |
| **光線追跡法 (Ray Tracing)** | フォトンショットの軌道計算 | ✅ 実装済み |
| **陽解法 (Explicit Method)** | パーティクルの位置・速度の時間発展 | ✅ 実装済み |
| **ポアソン方程式の簡略化** | 重力場の計算（ソフトニングパラメータを使用） | ✅ 実装済み |

### 関連する（将来拡張可能な）手法

| 数値計算法 | 用途 | 実装状況 |
|-----------|------|---------|
| **輻射輸送 (Radiative Transfer)** | より正確なホーキング放射のスペクトル計算 | 🔶 部分的実装（Python API） |
| **Tree法** | 大量パーティクルに対する高速重力計算 | ❌ 未実装（現状は直接計算） |
| **SPH法** | 流体としての降着円盤のシミュレーション | ❌ 未実装 |
| **FFT (PM法, TreePM法)** | 高速ポアソン方程式ソルバー | ❌ 未実装 |
| **マルチグリッド法** | ポアソン方程式の高精度解法 | ❌ 未実装 |

### 天体物理学との関連

原始星収縮、BH降着円盤、銀河形成、太陽磁場などの天体現象では、以下のような支配方程式と数値計算法が使用されます：

- **支配方程式**: 流体力学・磁気流体力学、ポアソン方程式、粒子の運動方程式、無衝突ボルツマン方程式、輻射輸送、熱伝導
- **数値計算法**: 格子法、SPH法、FFT、Tree法、粒子法、光線追跡法、陰解法・陽解法

本シミュレーターも同様のアプローチを採用しており、特に：
- **BH降着円盤**に関連する手法（輻射輸送、流体力学）を部分的に実装
- **粒子の運動方程式**を直接実装（ブラックホール周辺の軌道計算）
- **光線追跡法**を実装（フォトンショット）

## 物理定数

```javascript
// constants.js
G = 6.67430e-11      // 重力定数 (m³/kg/s²)
c = 299792458        // 光速 (m/s)
hbar = 1.054571817e-34 // 換算プランク定数 (J·s)
kB = 1.380649e-23    // ボルツマン定数 (J/K)
M_sun = 1.98847e30    // 太陽質量 (kg)
h = 6.62607015e-34   // プランク定数 (J·s)
```

## ブラックホールの物理量計算

### シュヴァルツシルト半径

ブラックホールの事象の地平線の半径を計算します。

```javascript
// JavaScript版 (constants.js)
schwarzschildRadius(Mkg) {
  return 2 * G * Mkg / (c * c);
}
```

```python
# Python版 (api/server.py)
def schwarzschild_radius(M_kg: float) -> float:
    """シュヴァルツシルト半径を計算"""
    return 2 * G * M_kg / (c * c)
```

**物理式**: `r_s = 2GM/c²`

- `M_kg`: ブラックホールの質量（kg）
- 返り値: シュヴァルツシルト半径（m）

### ホーキング温度

ブラックホールが放射する温度を計算します。

```javascript
// JavaScript版 (constants.js)
hawkingTemperature(Mkg) {
  return (hbar * Math.pow(c, 3)) / (8 * Math.PI * G * Mkg * kB);
}
```

```python
# Python版 (api/server.py)
def hawking_temperature(M_kg: float) -> float:
    """ホーキング温度を計算"""
    return (hbar * (c ** 3)) / (8 * math.pi * G * M_kg * kB)
```

**物理式**: `T_H = (ℏc³)/(8πGMk_B)`

- `M_kg`: ブラックホールの質量（kg）
- 返り値: ホーキング温度（K）

### 相対的な放射パワー

太陽質量に対する相対的なホーキング放射パワーを計算します。

```javascript
// JavaScript版 (constants.js)
relativePowerVsSolar(Mkg) {
  const Ms = M_sun;
  return Math.pow(Ms / Mkg, 2);
}
```

```python
# Python版 (api/server.py)
def relative_power_vs_solar(M_kg: float) -> float:
    """太陽質量に対する相対的な放射パワーを計算"""
    return (M_sun / M_kg) ** 2
```

**物理式**: `P_rel = (M_☉/M)²`

- 質量が小さいほど放射パワーが大きくなる（逆二乗則）

### 絶対的な放射パワー（Stefan-Boltzmann則）

```python
# Python版 (api/server.py)
def hawking_power_absolute(M_kg: float) -> float:
    """
    ホーキング放射の絶対的な放射パワーを計算（Stefan-Boltzmann則を使用）
    P = σ * A * T^4, where A = 4πr_s^2
    """
    rs = schwarzschild_radius(M_kg)
    temp = hawking_temperature(M_kg)
    sigma = 5.670374419e-8  # Stefan-Boltzmann constant
    area = 4 * math.pi * rs * rs
    return sigma * area * (temp ** 4)
```

**物理式**: `P = σA T_H⁴`, `A = 4πr_s²`

### Planck分布によるスペクトル計算

```python
# Python版 (api/server.py)
def planck_spectrum(frequency_hz: float, temperature_K: float) -> float:
    """
    プランク分布によるエネルギースペクトル密度を計算
    B_ν(T) = (2hν³/c²) / (exp(hν/kT) - 1)
    """
    nu = frequency_hz
    T = temperature_K
    
    exponent = (h * nu) / (kB * T)
    numerator = 2 * h * (nu ** 3) / (c ** 2)
    denominator = math.exp(exponent) - 1.0
    
    return numerator / denominator
```

**物理式**: `B_ν(T) = (2hν³/c²) / (exp(hν/kT) - 1)`

### 一般相対論的補正を考慮した重力加速度

```python
# Python版 (api/server.py)
def gravitational_acceleration(r: float, M_kg: float, rs: float) -> float:
    """
    一般相対論的効果を考慮した重力加速度（簡略版）
    Newton: a = -GM/r²
    相対論的補正（簡略）: a ≈ -GM/r² * (1 + 3rs/r) for r >> rs
    """
    newtonian = -G * M_kg / (r * r)
    correction = 1.0 + 3.0 * rs / r if r > rs * 1.1 else 1.0
    return newtonian * correction
```

**物理式**: 
- ニュートン重力: `a = -GM/r²`
- 相対論的補正: `a ≈ -GM/r² × (1 + 3r_s/r)` (r >> r_s の近似)

## パーティクル物理計算

### 重力加速度の計算

```javascript
// main.js - updateParticles()
const rsVisual = bhMesh.scale.x;
const soft = rsVisual * rsVisual * 0.4 + 0.04;  // ソフトニングパラメータ
const Gvis = 4.0 * rsVisual;                    // 視覚的な重力定数

// ブラックホールからの距離
const r2 = x*x + y*y + z*z;
const r = Math.sqrt(r2) + 1e-6;

// 重力加速度の計算
const accMag = -Gvis / (r2 + soft);
const invR = 1.0 / r;
const ax = accMag * x * invR;
const ay = accMag * y * invR;
const az = accMag * z * invR;
```

**物理式**: `a = -G_vis / (r² + ε)`

- `G_vis`: 視覚的な重力定数（4.0 × r_s）
- `soft`: ソフトニングパラメータ（特異点回避）
- `r`: ブラックホールからの距離

### 速度の減衰（種別に応じて）

```javascript
// main.js - updateParticles()
const sp = species[i];
const damping = (sp === 0) ? 0.998 :      // フォトン: 0.998
                (sp === 1) ? 0.9995 :     // ニュートリノ: 0.9995
                              0.999;       // グラビトン: 0.999

velocities[i3+0] *= damping;
velocities[i3+1] *= damping;
velocities[i3+2] *= damping;
```

各粒子種別に応じた減衰係数：
- **フォトン (γ)**: 0.998（最も減衰しやすい）
- **ニュートリノ (ν)**: 0.9995（中程度）
- **グラビトン (g)**: 0.999（最も減衰しにくい）

### パーティクル生成率の計算

```javascript
// main.js - spawnParticles()
const baseRate = 500.0;  // ベース生成率（ペア/秒）
const rate = baseRate * rateUI * Math.max(0.01, relPower);
```

**物理式**: `rate = baseRate × pairRateUI × max(0.01, P_rel)`

- `baseRate`: 500.0 ペア/秒
- `pairRateUI`: UI上のペア生成率（0.0-1.0）
- `relPower`: 相対的な放射パワー

## フォトンショットの物理計算

### 光の軌道計算（重力レンズ効果）

```javascript
// main.js - createPhotonBeam()
let pos = startPos.clone();
let vel = direction.clone().normalize().multiplyScalar(stepSize * 150);

const rsVisual = bhMesh.scale.x;
const Gvis = 4.0 * rsVisual;
const soft = rsVisual * rsVisual * 0.4 + 0.04;

for (let i = 0; i < steps; i++) {
  const r2 = pos.x * pos.x + pos.y * pos.y + pos.z * pos.z;
  const r = Math.sqrt(r2) + 1e-6;
  
  // 重力による曲がり
  const accMag = -Gvis / (r2 + soft);
  const invR = 1.0 / r;
  const ax = accMag * pos.x * invR;
  const ay = accMag * pos.y * invR;
  const az = accMag * pos.z * invR;
  
  // 速度の方向を更新（重力の影響）
  const pullStrength = 1.0 + (rsVisual / r) * 2.0;
  vel.x += ax * stepSize * 0.15 * pullStrength;
  vel.y += ay * stepSize * 0.15 * pullStrength;
  vel.z += az * stepSize * 0.15 * pullStrength;
  
  // 速度を正規化（光速を保つ）
  vel.normalize().multiplyScalar(stepSize * 150);
  
  // 位置を更新
  pos.add(vel.clone().multiplyScalar(stepSize));
}
```

**物理式**: 
- 重力加速度: `a = -G_vis / (r² + ε)`
- 引き寄せ強度: `pullStrength = 1 + 2r_s/r`
- 光速の維持: `|v| = const`

### ブラックホールへの落下

```javascript
// main.js - createPhotonBeam()
// 事象の地平線に到達したら、そこから先は吸い込まれる
if (r < rsVisual * 1.05) {
  const toCenter = new THREE.Vector3(-pos.x, -pos.y, -pos.z).normalize();
  const fallSpeed = (rsVisual * 1.05 - r) * 5.0;
  
  // ブラックホールの中心に向かって吸い込まれる様子を表現
  for (let fall = 0; fall < 10; fall++) {
    pos.add(toCenter.clone().multiplyScalar(fallSpeed * 0.1));
    // ...
  }
}
```

事象の地平線（`r < 1.05 × r_s`）を超えると、ブラックホールの中心に向かって加速的に落下します。

## エネルギーの統計的分布

### Gamma(4)分布によるサンプリング

```javascript
// constants.js
sampleXGamma4(rand01) {
  let x = 0;
  for(let i=0; i<4; i++) {
    const u = Math.max(1e-12, rand01());
    x += -Math.log(u);
  }
  return x; // mean 4
}
```

**物理的意味**: 黒体放射の `x³/(exp(x)-1)` の近似として、4つの指数分布の和（Erlang分布）を使用。

### エネルギーからRGBへの変換

```javascript
// constants.js
energyToRgb(E) {
  const lambda = (h * c) / E;  // 波長（メートル）
  const nm = lambda * 1e9;     // ナノメートル
  
  // 可視光範囲（380-700nm）にクランプ
  const L = Math.max(380, Math.min(700, nm));
  
  // 簡易的なCIE色空間からRGBへの変換
  // ...
}
```

**物理式**: `λ = hc/E`

## 重力ポテンシャルメッシュ

```javascript
// main.js - createGravityWellMesh()
const potential = -1.0 / (r_from_bh + rsVisual * 0.5);
const height = potential * 8.0; // 視覚的なスケール
```

**物理式**: `φ = -1/r`（簡略化された重力ポテンシャル）

3Dメッシュで重力の井戸を可視化しています。

## 時空の歪みグリッド

```javascript
// main.js - createSpacetimeGrid()
const potential = -1.0 / (r + rsVisual * 0.5);
const z = potential * 3.0; // z方向の変位
```

重力による空間の歪みを、グリッドのz方向変位として表現しています。

## Python APIの高度な計算

Python APIでは、JavaScriptでは困難な計算を実装しています。計算資源の範囲内で実装可能な手法を採用しています。

### 数値積分による総パワー計算

```python
# api/server.py
def total_hawking_power_numerical(M_kg: float, frequency_range: tuple = (1e10, 1e30)) -> float:
    """
    数値積分を使用してホーキング放射の総パワーを計算
    より正確な計算（Planck分布の積分）
    """
    temp = hawking_temperature(M_kg)
    
    def integrand(nu):
        return planck_spectrum(nu, temp) * math.pi
    
    result, _ = integrate.quad(integrand, frequency_range[0], frequency_range[1], limit=1000)
    return result
```

SciPyの数値積分を使用して、Planck分布の全周波数にわたる積分を実行します。

### N体問題のソルバー（Tree法の簡略版）

大量パーティクルに対する高速重力計算を実装しています。

```python
# api/physics_advanced.py
class NBodySolver:
    def calculate_gravity_tree(self, particles, bh_mass, bh_pos=(0,0,0)):
        """
        Tree法を使用した重力計算（簡略版）
        - ブラックホールからの重力を主要項として計算
        - パーティクル間の相互作用は近傍のみ考慮（Tree法の簡略版）
        """
```

**計算量**: O(N²) → O(N log N)（完全なTree法では）の簡略版
- 実際のBarnes-Hut Tree法では階層構造を使うが、ここでは距離閾値を使用
- 計算資源の範囲内で実装可能

### SPH法（Smoothed Particle Hydrodynamics）

流体としての降着円盤のシミュレーションを実装しています。

```python
# api/physics_advanced.py
class SPHSimulator:
    def calculate_density(self, particles):
        """
        SPH法による密度計算
        - Cubic Splineカーネル関数を使用
        - 各パーティクルの密度を近傍パーティクルから計算
        """
```

**用途**: 降着円盤の流体シミュレーション（将来拡張用）

### 輻射輸送（FLD法）

Flux-Limited Diffusion (FLD) 近似による輻射フラックス計算を実装しています。

```python
# api/physics_advanced.py
class RadiativeTransfer:
    def flux_limited_diffusion(self, temperature, density, opacity):
        """
        FLD法による輻射フラックス計算
        - 光学的に薄い場合: Stefan-Boltzmann則
        - 光学的に厚い場合: 拡散近似
        """
```

**用途**: より正確なホーキング放射のスペクトル計算

### より正確なフォトン軌道計算（一般相対論的）

一般相対論的効果を考慮したフォトン軌道計算を実装しています。

```python
# api/physics_advanced.py
def calculate_photon_trajectory_relativistic(
    start_pos, direction, bh_mass, steps=500, step_size=0.1
):
    """
    シュヴァルツシルト解での測地線方程式（簡略版）
    - メトリックテンソルから計算（簡略化）
    - 強い重力場での補正を考慮
    """
```

**用途**: より正確な重力レンズ効果の計算

### 軌道積分器（RK法）

より正確な時間発展のための積分器を実装しています。

```python
# api/physics_advanced.py
class OrbitalIntegrator:
    def __init__(self, method='rk4'):
        """
        積分手法: 'euler', 'rk2', 'rk4'
        - オイラー法: 1次精度
        - 2次ルンゲ・クッタ法: 2次精度
        - 4次ルンゲ・クッタ法: 4次精度（簡略版）
        """
```

**用途**: より正確なパーティクルの軌道計算

### エネルギーの統計的分布サンプリング

```python
# api/server.py
def energy_distribution_sample(temperature_K: float, num_samples: int = 1000) -> List[float]:
    """
    ホーキング温度に基づくエネルギーの統計的分布を計算
    より正確なエネルギー分布のサンプリング
    """
    kT = kB * temperature_K
    # Planck分布からサンプリング（逆変換サンプリングの近似）
    # ...
```

Planck分布に基づく、より正確なエネルギー分布のサンプリングを実装しています。

## 視覚化のための簡略化

実際の物理計算では、以下の簡略化を行っています：

1. **ソフトニングパラメータ**: 重力計算の特異点回避のため `soft = r_s² × 0.4 + 0.04`
2. **視覚的な重力定数**: `G_vis = 4.0 × r_s`（実際のGとは異なる）
3. **減衰係数**: 粒子種別に応じた経験的な減衰（0.998-0.999）
4. **光速の近似**: フォトンショットでは `v ≈ 150 × stepSize` として扱う

## 参考文献

- Hawking, S. W. (1974). "Black hole explosions?". Nature 248 (5443): 30–31.
- Schwarzschild, K. (1916). "Über das Gravitationsfeld eines Massenpunktes nach der Einsteinschen Theorie". Sitzungsberichte der Königlich Preussischen Akademie der Wissenschaften. 7: 189–196.

